...
const size_t size_w = 4096;
const size_t size_h = 4096;

typedef int2 mytype;
typedef mytype  arr_t[size_w];
const mytype A_val = make_int2(1,2);
const mytype B_val = make_int2(1,2);
...
__global__ void matrix_add_2D(const arr_t * __restrict__ A, const arr_t * __restrict__ B, arr_t * __restrict__ C, const size_t sw, const size_t sh){
  ...
  size_t idy = threadIdx.x+blockDim.x*(size_t)blockIdx.x;
  size_t idx = threadIdx.y+blockDim.y*(size_t)blockIdx.y;
  mytype a = A[idx][idy];
  mytype b = A[idx][idy+1];

  if ((idx < sh) && (idy < sw)) C[idx][idy] = make_int2(a.x + b.x,a.y+b.y);
}
